  ...
  return (
    <div className="flex flex-col gap-4">
      <ResponsiveContainer width="100%" height={250}>
        {isMood ? (
          <LineChart data={metric.data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            <YAxis stroke="#78716c" domain={[1, 5]} ticks={moodTicks} tickFormatter={v => moodEmojis[Number(v) - 1]} />
            <Tooltip contentStyle={{ background: '#fafaf9', color: '#292524', borderColor: '#a8a29e' }} wrapperStyle={{ borderRadius: '0.75rem' }} formatter={v => moodEmojis[Number(v) - 1] || v} />
      ...
      <div className="mt-2 text-sm text-stone-600 dark:text-stone-300 flex flex-wrap gap-4">
        {isMood && <span>Average: {average(values)} {moodEmojis[Math.max(0, Math.min(4, average(values) - 1))]}</span>}
